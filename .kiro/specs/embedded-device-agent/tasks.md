...
- [x]  2. 实现配置层（Config_Loader + Pydantic 模型 + 工厂契约）
...
- [x]  3. 实现 LLM_Provider 底座（Base + Factory）
  ...
  - [x]* 3.3 编写 LLM 工厂单元测试
    ...
    - _Requirements: 12.3_
- [x]  4. 实现 DeviceIO 底座（Base + Factory + 后端）
...
- [x]  5. 实现 Memory / Retriever 底座（Base + Factory + 后端）
  ...
  - [x]  5.2 实现 LocalStoreRetriever（基于 LangGraph Store）
  ...
  - [x]* 5.3 编写 Retriever 单元测试

    - 测试 `type` 分发、召回相关性排序、跨能力分区
    - _Requirements: 9.2, 9.4, 18.3_
- [x]  6. 组装 Core_Services 并做底座集成测试

  - [x]  6.1 实现 CoreServices 组装
...
- [x]  8. 实现 Capability A 确定性核心工具（可离线单测）

  - [x]  8.1 实现 Trace_Filter
  ...
  - [x]* 8.2 编写 Trace_Filter 单元测试与属性测试
  ...
  - [x]* 8.4 编写 Trace_Parser 单元测试与 round-trip 属性测试
...
- [x]  12. 实现 Capability A Orchestrator 与调优闭环 subgraph
  ...
  - [x]* 12.2 编写 Orchestrator 工具装配单元测试

    - 验证子智能体 vs 普通工具区分、返回结果写入 state、错误处理路径
    - **Property 8: 确定性核心无副作用**（确定性工具作为普通工具挂载）
    - **Validates: Requirements 13.1, 13.2, 13.3, 13.4**
  - [x]  12.3 实现调优闭环 subgraph（intake→recall→plan→collect→filter_parse→analyze→decide→instrument/optimize→memorize/halt）
  ...
  - [x]  12.4 实现 Half_Auto 人在环中断与验证/知识沉淀
...
- [x]  13. 检查点 - 确保 Capability A 测试通过

  - Ensure all tests pass, ask the user if questions arise.
- [x]  14. 实现平台层（BaseCapability / CapabilityFactory / Router）

  - [x]  14.1 实现 BaseCapability 与 CapabilityFactory
  ...
  - [x]  14.2 将 PerfTuningCapability 封装并注册
...
- [x]  15. 实现 Capability B（FunctionalTest）契约与骨架

  - [x]  15.1 定义 FunctionalTest 数据契约

    - 在 `capabilities/functional_test/models.py` 定义测试用例、断言、测试报告的 Pydantic 契约（API 边界可语言中立暴露）
    - _Requirements: 19.3_
  - [x]  15.2 实现 FunctionalTestCapability 骨架与最小可跑流程
...
- [x]  16. 实现测试 Harness
  ...
  - [x]  16.1 实现 FakeDeviceIO 与 FakeLLMProvider
